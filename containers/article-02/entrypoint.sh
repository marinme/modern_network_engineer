#!/bin/bash
# Docker bind-mounts /proc/sys read-only by default. Article 2 needs to write
# to /proc/sys/net/ipv4/ip_forward (and friends) per network namespace, so
# remount it rw here. SYS_ADMIN (granted via --cap-add) is what makes this
# possible without --privileged.
#
