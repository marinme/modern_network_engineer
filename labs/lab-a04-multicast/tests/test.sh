#!/usr/bin/env bash
# test.sh — standard verification entrypoint for Lab A04 (multicast).
#
# Lab A04 multicast has two sub-labs (FRR pimd and smcroute static), so this
# dispatcher takes the part you want to verify and runs its checker.
# Each checker is verify-only and non-destructive.
#
# Usage
#   ./tests/multicast/test.sh 1           # by number
#   ./tests/multicast/test.sh 1-pim-sm    # by full slug
#   ./tests/multicast/test.sh pim-sm      # by name
#   ./tests/multicast/test.sh             # list the parts
#
...
usage() {
    ...
    echo "Verify a built Lab A04 multicast sub-lab (verify-only, non-destructive)."
    if [ "${#SCRIPTS[@]}" -gt 0 ]; then
        ...
        for s in "${SCRIPTS[@]}"; do
            ...
            num="${base%%-*}"; name="${base#*-}"
            printf "  %-2s  %-28s -> ./tests/multicast/test.sh %s\n" "$num" "$name" "$num"
        done
    else
